...
sTime = datetime(2020, 7, 20, 0, 0, 0);
eTime = datetime(2020, 7, 22, 0, 0, 0);
nTime = seconds(eTime - sTime) / tStep + 1;
lTime = sTime:seconds(tStep):eTime;
rTime = timerange(sTime, eTime, "closed");
%% Input Data
dataTbl = readtable('exampledata2.csv'); % Changed file name to exampledata2.csv
...
dataTT.Properties.VariableNames = {'T_a', 'V_w', 'W_d', 'Rad_solar'}; % Changed variable name to Rad_solar
...
dataTT.beta = beta;
% initial current
init_I = 1200; % Assuming maximum current is 1200 A, removed dependence on dataTT.dem
dataTT.I = zeros(size(dataTT, 1), 1);
var = {'T_a', 'V_w', 'beta', 'Rad_solar'}; % Added 'Rad_solar'
...
inputdata = table2array(removevars(timetable2table(inputTT), 'Time')); % Removed 'Time' from variables
...
Rad_solar = inputdata(:, 4); % Solar radiation
...
for iTime = sTime:seconds(tStep):eTime
    ...
    [Res.T_avg(cnt, 1), calculated_I(cnt), Res.R(cnt, 1)] = IEEEstd738_(Tavg(max(cnt - 1, 1), 1), T_abs(cnt), wind_vel(cnt), beta(cnt), t(cnt), ...
        Rad_solar(cnt), IEEEstd738Para); % Added Rad_solar as input
end
toc
%%
...
writetable(table_current, 'savedData2.csv'); % Changed output file name to savedData2.csv
%% Simulation result
